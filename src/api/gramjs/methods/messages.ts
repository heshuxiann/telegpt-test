...
export async function fetchMessages({
  ...
  const RequestClass =
    threadId === MAIN_THREAD_ID
      ...
      : isSavedDialog
      ? GramJs.messages.GetSavedHistory
...
export function sendMessageLocal(params: SendMessageParams) {
  const {
    chat,
    lastMessageId,
    text,
...
export function sendApiMessage(
  ...
  const messagePromise = (async () => {
    ...
    type SendMediaArgs = ConstructorParameters<
      typeof GramJs.messages.SendMedia
    >[0];
    type SendMessageArgs = ConstructorParameters<
      typeof GramJs.messages.SendMessage
    >[0];
    ...
    const args: SharedArgs = {
      ...
      allowPaidStars: messagePriceInStars
        ? BigInt(messagePriceInStars)
        : undefined,
      suggestedPost:
        suggestedPostInfo && buildInputSuggestedPost(suggestedPostInfo),
    };

    try {
      let update;
      if (media) {
        update = await invokeRequest(
          new GramJs.messages.SendMedia({
            ...args,
            media,
          }),
          {
...
export async function sendMessage(
  ...
  return localMessage
    ? sendApiMessage(params, localMessage, onProgress)
    : undefined;
}
  ...
  mediaQueue = (async () => {
    ...
    const update = await invokeRequest(
      new GramJs.messages.SendMultiMedia({
        ...
        ...(sendAs && { sendAs: buildInputPeer(sendAs.id, sendAs.accessHash) }),
        ...(messagePriceInStars && {
          allowPaidStars: BigInt(messagePriceInStars * count),
        }),
        ...(suggestedPostInfo && {
          suggestedPost: buildInputSuggestedPost(suggestedPostInfo),
        }),
      }),
      {
        shouldIgnoreUpdates: true,
...
export async function editMessage(
  ...
  const isInvertedMedia =
    text && !attachment?.shouldSendAsFile ? message.isInvertedMedia : undefined;
...
export async function appendTodoList({
  ...
  try {
    await invokeRequest(
      new GramJs.messages.AppendTodoList({
        peer: buildInputPeer(chat.id, chat.accessHash),
        msgId: message.id,
        list: todoItems,
      }),
...
export async function deleteSavedHistory({ chat }: { chat: ApiChat }) {
  const result = await invokeRequest(
    new GramJs.messages.DeleteSavedHistory({
      peer: buildInputPeer(chat.id, chat.accessHash),
      maxId: DEFAULT_PRIMITIVES.INT,
    })
  );
...
export async function toggleSuggestedPostApproval({
  ...
  const result = await invokeRequest(
    new GramJs.messages.ToggleSuggestedPostApproval({
      peer: buildInputPeer(chat.id, chat.accessHash),
      msgId: messageId,
...
export function reportChannelSpam({
  ...
  return invokeRequest(
    new GramJs.channels.ReportSpam({
      participant: buildInputPeer(peer.id, peer.accessHash),
      channel: buildInputChannel(chat.id, chat.accessHash),
      id: messageIds,
    })
...
export async function fetchFactChecks({
  ...
  const results = await Promise.all(
    chunks.map((chunkIds) =>
      invokeRequest(
        new GramJs.messages.GetFactCheck({
          peer: buildInputPeer(chat.id, chat.accessHash),
          msgId: chunkIds,
        })
      )
...
export function fetchPaidReactionPrivacy() {
  return invokeRequest(new GramJs.messages.GetPaidReactionPrivacy(), {
...
export function reportMessagesDelivery({
  ...
  return invokeRequest(
    new GramJs.messages.ReportMessagesDelivery({
      peer: buildInputPeer(chat.id, chat.accessHash),
      id: messageIds,
    })
  );
...
export async function searchMessagesGlobal({
  ...
  const result = await invokeRequest(
    new GramJs.messages.SearchGlobal({
      ...
      offsetId: offsetId ?? DEFAULT_PRIMITIVES.INT,
      broadcastsOnly:
        type === "channels" || context === "channels" || undefined,
  ...
  const nextOffsetRate =
    "nextRate" in result && result.nextRate ? result.nextRate : undefined;
...
export async function searchPublicPosts({
  ...
  const result = await invokeRequest(
    new GramJs.channels.SearchPosts({
      hashtag,
      query,
  ...
  const searchFlood =
    result instanceof GramJs.messages.MessagesSlice && result.searchFlood
...
export async function checkSearchPostsFlood(query?: string) {
  const result = await invokeRequest(
    new GramJs.channels.CheckSearchPostsFlood({ query })
  );
...
export async function toggleTodoCompleted({
  ...
  await invokeRequest(
    new GramJs.messages.ToggleTodoCompleted({
      peer: buildInputPeer(id, accessHash),
      msgId: messageId,
...
export async function fetchSponsoredMessages({ peer }: { peer: ApiPeer }) {
  const result = await invokeRequest(
    new GramJs.messages.GetSponsoredMessages({
      peer: buildInputPeer(peer.id, peer.accessHash),
    })
  );
  ...
  if (
    !result ||
    result instanceof GramJs.messages.SponsoredMessagesEmpty ||
    !result.messages.length
  ) {
    return undefined;
  }
...
export async function viewSponsoredMessage({ random }: { random: string }) {
  await invokeRequest(
    new GramJs.messages.ViewSponsoredMessage({
...
export function clickSponsoredMessage({
  ...
  return invokeRequest(
    new GramJs.messages.ClickSponsoredMessage({
...
export async function reportSponsoredMessage({
  ...
  try {
    const result = await invokeRequest(
      new GramJs.messages.ReportSponsoredMessage({
    ...
    if (
      ...
      err.errorMessage === "PREMIUM_ACCOUNT_REQUIRED"
    ) {
      return {
...
function handleMultipleLocalMessagesUpdate(
  ...
  const updateMessageIds = update.updates.filter(
  ...
  updateMessageIds.forEach((updateMessageId) => {
    const updateNewScheduledMessage = update.updates.find(
      (
        scheduledUpdate
...
function handleLocalMessageUpdate(
  ...
  if (
    ...
    scheduledMessageUpdate = update.updates.find(
      (u): u is GramJs.UpdateNewScheduledMessage =>
    ...
    if (messageUpdate.media) {
      newContent = {
        ...
        ...buildMessageMediaContent(messageUpdate.media, {
          peerId: buildPeer(localMessage.chatId),
          id: messageUpdate.id,
        }),
      };
  ...
  const newScheduledMessage =
    scheduledMessageUpdate?.message &&
    buildApiMessage(scheduledMessageUpdate.message);

  // Edge case for "Send When Online"
  const isSentBefore =
    "date" in messageUpdate && messageUpdate.date < getServerTime();
...
export async function fetchOutboxReadDate({
  ...
  const result = await invokeRequest(
    new GramJs.messages.GetOutboxReadDate({
      peer,
      msgId: messageId,
    }),
    { shouldThrow: true }
...
export async function fetchQuickReplies() {
  const result = await invokeRequest(
    new GramJs.messages.GetQuickReplies({
      hash: DEFAULT_PRIMITIVES.BIGINT,
    })
  );
...
export async function sendQuickReply({
  ...
  const messages = await invokeRequest(
    new GramJs.messages.GetQuickReplyMessages({
      shortcutId,
      hash: DEFAULT_PRIMITIVES.BIGINT,
    })
  );
  ...
  const result = await invokeRequest(
    new GramJs.messages.SendQuickReplyMessages({
      peer: buildInputPeer(chat.id, chat.accessHash),
      shortcutId,
      id: ids,
      randomId: randomIds,
  ...
  if ("updates" in result) {
    const filteredUpdates = result.updates.filter(
      (u): u is GramJs.UpdateMessageID => !(u instanceof GramJs.UpdateMessageID)
    );
...
export async function exportMessageLink({
  ...
  const result = await invokeRequest(
    new GramJs.channels.ExportMessageLink({
      channel: buildInputChannel(chat.id, chat.accessHash),
      id,
...
export async function fetchPreparedInlineMessage({
  ...
  const result = await invokeRequest(
    new GramJs.messages.GetPreparedInlineMessage({
