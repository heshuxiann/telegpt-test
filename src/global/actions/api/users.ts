...
import {
  ChataiContactStore,
  ChataiUserStore,
} from '../../../components/chatAssistant/store';
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (
      ...
      && (isPreload || !profilePhotos.nextOffset)
    ) return;
    ...
    const limit = !offset || isPreload || shouldInvalidateCache
      ? PROFILE_PHOTOS_FIRST_LOAD_LIMIT
      : undefined;
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const newPeerIds = unique([peerId, ...currentPeerIds]).slice(
      0,
      BOT_VERIFICATION_PEERS_LIMIT,
    );
